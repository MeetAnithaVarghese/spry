-- SQLITE ETL SCRIPT — FINAL VERSION WITH SIMPLIFIED STATIC FIELD EXTRACTION
...
-- 4. NORMALIZE + ROLE ATTACH & CODE EXTRACTION (12 PARENTHESES AND SIMPLIFIED ARITHMETIC)
------------------------------------------------------------------------------
...
SELECT
    ...
    -- Extract @id (Test Case ID) - SIMPLIFIED EXTRACTION
    CASE WHEN INSTR(s.body_json_string, '@id') > 0 THEN
        TRIM(
            SUBSTR(
                s.body_json_string,
                INSTR(s.body_json_string, '@id') + 4,
                INSTR(s.body_json_string, '"', INSTR(s.body_json_string, '@id') + 4) - (INSTR(s.body_json_string, '@id') + 4)
            )
        )
    ELSE NULL
    END AS extracted_id,
    ...
    CASE 
        WHEN INSTR(s.body_json_string, '"code":"') > 0 THEN
            -- CRITICAL FIX: Inject CHAR(10) before ALL known keys to force separation
            REPLACE( -- 12
                REPLACE( -- 11
                    REPLACE( -- 10
                        REPLACE( -- 9
                                                    ...
                                                    REPLACE( -- 2
                                                        REPLACE( -- 1
                                                            ...
                                                            SUBSTR(
                                                                ...
                                                                -- CRITICAL FIX: Simplified arithmetic from previous step is preserved
                                                                INSTR(s.body_json_string, '","type":') - INSTR(s.body_json_string, '"code":"') - 8
                                                            ), -- 1st closing parenthesis (for SUBSTR)
                                                            
                                                            -- MANDATORY HEADER CLEANUP
                                                            'doc-classify:', CHAR(10) || 'doc-classify:'
                                                        ), -- 2nd closing parenthesis (for REPLACE 1)
                                                        'role:', CHAR(10) || 'role:'
                                                    ), -- 3rd closing parenthesis (for REPLACE 2)
                                                    
                                                    -- CASE keys
                                                    'TestCaseStatus:', CHAR(10) || 'TestCaseStatus:' 
                                                ), -- 4th closing parenthesis (for REPLACE 3)
                                                'Tags:', CHAR(10) || 'Tags:'
                                            ), -- 5th closing parenthesis (for REPLACE 4)
                                            'Scenario Type:', CHAR(10) || 'Scenario Type:' 
                                        ), -- 6th closing parenthesis (for REPLACE 5)
                                        'Priority:', CHAR(10) || 'Priority:' 
                                    ), -- 7th closing parenthesis (for REPLACE 6)
                                    'requirementID:', CHAR(10) || 'requirementID:'
                                ), -- 8th closing parenthesis (for REPLACE 7)
                                -- EVIDENCE keys
                                'cycle:', CHAR(10) || 'cycle:'
                            ), -- 9th closing parenthesis (for REPLACE 8)
                            'severity:', CHAR(10) || 'severity:' -- CRITICAL FOR EVIDENCE
                        ), -- 10th closing parenthesis (for REPLACE 9)
                        'assignee:', CHAR(10) || 'assignee:'
                    ), -- 11th closing parenthesis (for REPLACE 10)
                    'status:', CHAR(10) || 'status:'
                ), -- 12th closing parenthesis (for REPLACE 11)
    ...
    -- STATIC FIELD EXTRACTIONS (CRITICAL FIX: SIMPLIFYING LOGIC)
    
    -- Extract TestCaseStatus
    CASE
        -- Logic for "TestCaseStatus:"
        WHEN s.body_json_string LIKE '%TestCaseStatus:%' THEN
            TRIM(SUBSTR(
                s.body_json_string,
                INSTR(s.body_json_string, 'TestCaseStatus:') + 15,
                INSTR(s.body_json_string, '"', INSTR(s.body_json_string, 'TestCaseStatus:') + 15) - (INSTR(s.body_json_string, 'TestCaseStatus:') + 15)
            ))
        -- Fallback for "testcasestatus:"
        WHEN s.body_json_string LIKE '%testcasestatus:%' THEN 
            TRIM(SUBSTR(
                s.body_json_string,
                INSTR(s.body_json_string, 'testcasestatus:') + 15,
        ...
        ELSE NULL
    END AS static_test_case_status,
    
    -- Extract Priority
    CASE
        -- Logic for "Priority:"
        WHEN s.body_json_string LIKE '%Priority:%' THEN
            TRIM(SUBSTR(
                s.body_json_string,
                INSTR(s.body_json_string, 'Priority:') + 9,
                INSTR(s.body_json_string, '"', INSTR(s.body_json_string, 'Priority:') + 9) - (INSTR(s.body_json_string, 'Priority:') + 9)
            ))
        ...
        ELSE NULL
    END AS static_priority,
    
    -- Extract Scenario Type
    CASE
        -- Logic for "Scenario Type:"
        WHEN s.body_json_string LIKE '%Scenario Type:%' THEN
            TRIM(SUBSTR(
                s.body_json_string,
                INSTR(s.body_json_string, 'Scenario Type:') + 14,
                INSTR(s.body_json_string, '"', INSTR(s.body_json_string, 'Scenario Type:') + 14) - (INSTR(s.body_json_string, 'Scenario Type:') + 14)
            ))
        -- Fallback for "scenario type:"
        WHEN s.body_json_string LIKE '%scenario type:%' THEN 
            TRIM(SUBSTR(
                s.body_json_string,
                INSTR(s.body_json_string, 'scenario type:') + 14,
...
-- 4.5. ISOLATE TEST CASES (STATIC DETAILS) - NEW TABLE
------------------------------------------------------------------------------

DROP TABLE IF EXISTS t_testcases;
CREATE TABLE t_testcases AS
SELECT
    uniform_resource_id,
    file_basename,
    extracted_id AS test_case_id,
    title AS test_case_title,
    body_json_string AS raw_body_json, -- Contains the description
    static_test_case_status,
    static_priority,
    static_scenario_type
FROM t_all_sections
WHERE role_name = 'case' AND extracted_id IS NOT NULL;
...
-- 6. TEST CASE DETAILS (Aggregates static case details and dynamic evidence status)
------------------------------------------------------------------------------
...
SELECT
    ...
    tc.test_case_id,
    tc.test_case_title,
    tc.raw_body_json, -- Contains the description
    
    -- Static Details (from case YAML)
    tc.static_test_case_status AS static_status,
    tc.static_priority AS static_priority,
    tc.static_scenario_type AS static_scenario_type,
    
    -- Dynamic Evidence Status (from latest evidence record)
    les.latest_status AS evidence_status, 
    les.latest_severity AS evidence_severity,
    les.latest_cycle,
    les.latest_assignee,
    les.latest_issue_id
FROM t_testcases tc
LEFT JOIN t_latest_evidence_status les
    ON tc.uniform_resource_id = les.uniform_resource_id
   AND tc.test_case_id = les.test_case_id;
