-- SQLITE ETL SCRIPT — FINAL VERSION WITH t_testcases TABLE AND CORRECT 16 PARENTHESES COUNT
...
-- 4. NORMALIZE + ROLE ATTACH & CODE EXTRACTION (CORRECTED PARENTHESES: 16 TOTAL)
------------------------------------------------------------------------------
...
SELECT
    ...
    CASE 
        WHEN INSTR(s.body_json_string, '"code":"') > 0 THEN
            ...
            )))))))))))))))) -- <--- FINAL CORRECTION: 16 CLOSING PARENTHESES
        ELSE NULL
    END AS code_content,
    
    rm.role_name,
    
    -- STATIC FIELD EXTRACTIONS (RE-INTRODUCED for t_testcases)
    
    -- Extract TestCaseStatus
    CASE
        WHEN s.body_json_string LIKE '%TestCaseStatus:%' THEN
            TRIM(SUBSTR(
                SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'TestCaseStatus:') + 15),
                1,
                INSTR(SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'TestCaseStatus:') + 15), '"') - 1
            ))
        WHEN s.body_json_string LIKE '%testcasestatus:%' THEN -- Fallback for lower case
            TRIM(SUBSTR(
                SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'testcasestatus:') + 15),
                1,
                INSTR(SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'testcasestatus:') + 15), '"') - 1
    ...
    CASE
        WHEN s.body_json_string LIKE '%Priority:%' THEN
            TRIM(SUBSTR(
                SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'Priority:') + 9),
                1,
                INSTR(SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'Priority:') + 9), '"') - 1
            ))
        WHEN s.body_json_string LIKE '%priority:%' THEN -- Fallback for lower case
            TRIM(SUBSTR(
                SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'priority:') + 9),
                1,
                INSTR(SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'priority:') + 9), '"') - 1
    ...
    CASE
        WHEN s.body_json_string LIKE '%Scenario Type:%' THEN
            TRIM(SUBSTR(
                SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'Scenario Type:') + 14),
                1,
                INSTR(SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'Scenario Type:') + 14), '"') - 1
            ))
        WHEN s.body_json_string LIKE '%scenario type:%' THEN -- Fallback for lower case
            TRIM(SUBSTR(
                SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'scenario type:') + 14),
                1,
                INSTR(SUBSTR(s.body_json_string, INSTR(s.body_json_string, 'scenario type:') + 14), '"') - 1
