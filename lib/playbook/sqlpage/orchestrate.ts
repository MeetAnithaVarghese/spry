        ...
        switch (purpose) {
          ...
          case "partial":
            return {
              ...
              raw: rawSQL,
              ...(spfu.cell
                ? (isStorable(spfu.cell) ? spfu.cell.storableAttrs : {})
                : {}),
              ...spfu,
            };
        }
...
export async function* sqlPageFiles(
  ...
  const { sources, routes, partials, storables, routeAnnsF, directives } =
    playbook;
    ...
    for (const handler of handlers) {
      ...
      if (content) {
        ...
        if (spf.cell) {
          ...
          // }
          if (
            isStorable(cell) && cell.storableAttrs &&
            Object.entries(cell.storableAttrs).length
          ) {
            yield jsonSPF(
